...
switch (argv._[0]) {
	...
	case 'migrate':
	case 'migrate:up':
		migrate.up(argv, function() {
			process.exit(1);
		});
		break;
	case 'migrate:down':
		migrate.down(argv, function() {
			process.exit(1);
		});
		break;
	case 'migrate:reset':
		migrate.reset(argv, function() {
			process.exit(1);
		});
		break;
	case 'migrate:setup':
	...
	fs.exists('./' + folder, function(exists) {
		...
		fs.exists('./' + folder + '/db.conf.js', function(exists) {
			if (exists) {
				...
				fs.writeFile('./' + folder + '/db.conf.js', dbConfSync(), function(err) {
					if (err) throw err;
